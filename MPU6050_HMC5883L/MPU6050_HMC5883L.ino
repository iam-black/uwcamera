...
//SoftwareSerial mySerial(10, 11); // debug RX, TX

#define RESTRICT_PITCH // Comment out to restrict roll to В±90deg instead - please read: http://www.freescale.com/files/sensors/doc/app_note/AN3461.pdf
...
void setup() {
  ...
  i2cData[2] = 0x00; // Set Gyro Full Scale Range to В±250deg/s
  i2cData[3] = 0x00; // Set Accelerometer Full Scale Range to В±2g
...
void loop() {
   ...
   //filtering in case some giberish is received
   while(strReceive.charAt(0) != 'r' && strReceive.charAt(0) != 'l' && strReceive.charAt(0) != 'u' && strReceive.charAt(0) != 'd'
   && strReceive.length() > 0)
        {
         strReceive.remove(0,1);
        }
   
   if(strReceive.charAt(0) == 'l'){
       //mySerial.print(strReceive.charAt(0));
     ...
     if(strReceive.charAt(0) == 'd'){
        while(strReceive.charAt(0) == 'd')
        {
          servoPosV += 1;
          strReceive.remove(0,1);
        }         
        servoPosV = constrain(servoPosV, 60,170);
        if(servoPosV<181)
        {
          servoV.write(servoPosV++);
          //mySerial.print(servoPosV);
          //mySerial.print('\n');
        delay(10);  
        }
      ...
      if(strReceive.charAt(0) == 'u'){
         while(strReceive.charAt(0) == 'u')
        {
          servoPosV -= 1;
          strReceive.remove(0,1);
        }  
 ...
 // Serial.print(gyroXangle); Serial.print("\t");
  ...
  Serial.print("x");
  //Serial.print((int)round(kalAngleX)); //Serial.print("\t");
  Serial.print(constrain(kalAngleX,-179,179),1);
  Serial.print("xx");
 ...
 // Serial.print(compAngleY); Serial.print("\t");
  Serial.print("y");
  Serial.print(constrain(kalAngleY,-179,179),1); //Serial.print("\t");
  Serial.print("yy");
 ...
 // Serial.print(compAngleZ); Serial.print("\t");
  Serial.print("z");
  Serial.print(constrain(kalAngleZ,-179,179),1); //Serial.print("\t");
  Serial.print("zz");
#endif
...
void updatePitchRoll() {
  ...
  // atan2 outputs the value of -ПЂ to ПЂ (radians) - see http://en.wikipedia.org/wiki/Atan2
